...
function bindEvents() {
  ...
  if (clearDoneBtn) clearDoneBtn.onclick = () => {
    shopping = shopping.filter(item => !item.done);
    save();
    renderShopping();
  };
...
function handleGlobalCardButtons(e) {
  const cartBtn = e.target.closest(".quick-add");
  if (cartBtn) {
    e.preventDefault();
    e.stopPropagation();
    const r = allRecipes.find(x => x.id === cartBtn.dataset.id);
...
function openRecipe(r) {
  ...
  inner.innerHTML = `
    ${image}
    <button class="modal-float-cart" onclick="addIngredientsToShoppingById('${r.id}')" title="Добавить ингредиенты">🛒</button>
    <div class="modal-content">
      ...
      <h3>Как готовить</h3>
      <ol class="steps-list">${(r.steps || []).map(x => `<li>${x}</li>`).join("")}</ol>
...
function addIngredientsToShopping(r) {
  ...
  renderShopping();
  try {
    const btn = document.querySelector(`.quick-add[data-id="${r.id}"]`) || document.querySelector(".modal-float-cart");
    if (btn) {
      btn.animate([{ transform: "scale(1)" }, { transform: "scale(1.18)" }, { transform: "scale(1)" }], { duration: 220 });
    }
  } catch(e) {}
}
...
function switchTab(tab) {
  ...
  document.querySelectorAll(".home-only").forEach(el => {
    el.classList.toggle("hidden", tab !== "home");
  });

  const panels = ["homeSection", "favoritesSection", "cookAgainSection", "fridgeSection", "shoppingSection", "addSection"];
  panels.forEach(id => {
