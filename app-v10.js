...
function switchTab(tab) {
  ...
  document.getElementById("recentPanel").classList.toggle("hidden", tab !== "home" || recent.length === 0);
  document.getElementById("trendingPanel").classList.toggle("hidden", tab !== "home");
...
function renderRecent() {
  ...
  panel.classList.toggle("hidden", currentTab !== "home" || list.length === 0);
  box.innerHTML = list.map(miniCardHtml).join("");
}

function renderTrending() {
  const box = document.getElementById("trendingGrid");
  if (!box) return;
  const list = [...allRecipes]
    .sort((a,b) => (cookStats[b.id] || 0) - (cookStats[a.id] || 0))
    .filter(r => (cookStats[r.id] || 0) > 0)
    .slice(0, 10);
  const fallback = allRecipes.filter(r => (r.badges || []).some(b => b.includes("быстро") || b.includes("comfort"))).slice(0, 10);
  box.innerHTML = (list.length ? list : fallback).map(miniCardHtml).join("");
}
